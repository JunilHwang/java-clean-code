...
import me.cleancode.lotto.step2.exception.LottoGamePriceException;
import me.cleancode.lotto.step2.domain.LottoShop;
...
public class LottoShopTest {
  ...
  void 구매_금액_검증_테스트 (int price) {
    assertThatExceptionOfType(LottoGamePriceException.class)
      .isThrownBy(() -> LottoShop.buyLotto(price));
  }
  ...
  void 구매_갯수_테스트 (long price, long expected) {
    Assertions.assertEquals(expected, LottoShop.buyLotto(price).stream().count());
  }
