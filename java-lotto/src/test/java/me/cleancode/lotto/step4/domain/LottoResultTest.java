package me.cleancode.lotto.step4.domain;

import me.cleancode.lotto.step3.domain.Lotto;
import me.cleancode.lotto.step3.domain.LottoNumber;
import me.cleancode.lotto.step3.domain.LottoResult;
import me.cleancode.lotto.step3.domain.Lottos;
import me.cleancode.lotto.step3.domain.Prize;
import me.cleancode.lotto.step3.domain.Rank;
import me.cleancode.lotto.step3.domain.WinningLotto;
import me.cleancode.lotto.step3.domain.LottoGenerator;
import org.junit.jupiter.api.Assertions;
import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.params.ParameterizedTest;
import org.junit.jupiter.params.provider.Arguments;
import org.junit.jupiter.params.provider.MethodSource;

import java.util.stream.Stream;

import static java.util.stream.Collectors.toList;
import static me.cleancode.lotto.step3.domain.LottoGenerator.generateLotto;
import static org.junit.jupiter.api.Assertions.assertEquals;

public class LottoResultTest {

  private static final me.cleancode.lotto.step3.domain.LottoResult LOTTO_RESULT = me.cleancode.lotto.step3.domain.LottoResult.of(
    me.cleancode.lotto.step3.domain.Lottos.of(Stream.of(
      LottoGenerator.generateLotto("1,2,3,4,5,7"),
      LottoGenerator.generateLotto("1,2,3,4,5,8"),
      LottoGenerator.generateLotto("1,2,3,40,20,10"),
      LottoGenerator.generateLotto("1,2,3,30,20,10")
    ).collect(toList())),
    me.cleancode.lotto.step3.domain.WinningLotto.of(
      LottoGenerator.generateLotto("1,2,3,4,5,6"),
      me.cleancode.lotto.step3.domain.LottoNumber.of("7")
    )
  );

  @DisplayName("로또 당첨 수를 구하는 메소드 검증")
  @ParameterizedTest
  @MethodSource("provideWinningLottoCount")
  void 로또_당첨수_테스트 (me.cleancode.lotto.step3.domain.LottoResult lottoResult, me.cleancode.lotto.step3.domain.Rank rank, long expected) {
    Assertions.assertEquals(
      lottoResult.stream()
        .filter(prize -> prize.getRank().equals(rank))
        .map(Prize::getCount).findFirst().orElse(0L),
      expected
    );
  }

  private static Stream<Arguments> provideWinningLottoCount () {
    return Stream.of(
      Arguments.of(LOTTO_RESULT, me.cleancode.lotto.step3.domain.Rank.SECOND, 1),
      Arguments.of(LOTTO_RESULT, me.cleancode.lotto.step3.domain.Rank.THIRD, 1),
      Arguments.of(LOTTO_RESULT, me.cleancode.lotto.step3.domain.Rank.FIFTH, 2)
    );
  }

  @DisplayName("로또 당첨 금액을 구하는 테스트")
  @Test
  void 로또_당첨_금액_테스트 () {
    long expected = me.cleancode.lotto.step3.domain.Rank.SECOND.getPrice() + me.cleancode.lotto.step3.domain.Rank.THIRD.getPrice() + (me.cleancode.lotto.step3.domain.Rank.FIFTH.getPrice() * 2);
    assertEquals(
      LOTTO_RESULT.getPayoff(),
      expected
    );
  }
  private static final String RATIO_FORMAT = "%.2f";
  private static final me.cleancode.lotto.step3.domain.LottoNumber BONUS_NUMBER = LottoNumber.of("7");
  private static final me.cleancode.lotto.step3.domain.Lottos lottos = Lottos.of(
    Stream.of(
      LottoGenerator.generateLotto("1,2,3,4,5,6"),
      LottoGenerator.generateLotto("1,2,3,4,5,7"),
      LottoGenerator.generateLotto("1,2,3,4,5,10"),
      LottoGenerator.generateLotto("1,2,3,4,20,10"),
      LottoGenerator.generateLotto("1,2,3,30,20,10")
    ).collect(toList())
  );

  @DisplayName("로또 당첨 금액의 수익률 검증 테스트")
  @ParameterizedTest
  @MethodSource("provideLottoGameAndWinningPrice")
  void 로또_당첨_수익률_테스트 (me.cleancode.lotto.step3.domain.LottoResult lottoResult, String expected) {
    String payoffRatio = String.format(RATIO_FORMAT, lottoResult.resultLottoGamePayOffRatio());
    assertEquals(expected, payoffRatio);
  }

  private static Stream<Arguments> provideLottoGameAndWinningPrice () {
    final long payoff = me.cleancode.lotto.step3.domain.Rank.stream().mapToLong(Rank::getPrice).sum();
    final String payoffRatio1 = String.format(RATIO_FORMAT, payoff / (5.0 * Lotto.PRICE));
    final String payoffRatio2 = "0.00";
    return Stream.of(
      Arguments.of(
        me.cleancode.lotto.step3.domain.LottoResult.of(
          lottos,
          me.cleancode.lotto.step3.domain.WinningLotto.of(LottoGenerator.generateLotto("1,2,3,4,5,6"), BONUS_NUMBER)
        ),
        payoffRatio1
      ),
      Arguments.of(
        LottoResult.of(
          lottos,
          WinningLotto.of(LottoGenerator.generateLotto("11,12,13,14,15,16"), BONUS_NUMBER)
        ),
        payoffRatio2
      )
    );
  }
}
