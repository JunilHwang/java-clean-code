...
import me.cleancode.lotto.step2.domain.Lotto;
import me.cleancode.lotto.step2.domain.LottoGenerator;
import me.cleancode.lotto.step2.exception.InvalidRangeNumberException;
import me.cleancode.lotto.step2.exception.LottoCountException;
...
public class LottoTest {
  ...
  void 로또_번호_갯수_검증 (String lottoNumbers) {
    assertThatExceptionOfType(LottoCountException.class)
      .isThrownBy(() -> me.cleancode.lotto.step2.domain.LottoGenerator.generateLotto(lottoNumbers));
  }
  ...
  void 로또_번호_중복_제거 (String lottoNumbers) {
    ...
    me.cleancode.lotto.step2.domain.Lotto lotto = me.cleancode.lotto.step2.domain.LottoGenerator.generateLotto(lottoNumbers);
  ...
  void 로또_번호_오름차순_검사 (me.cleancode.lotto.step2.domain.Lotto lotto, String expected) {
    assertEquals(expected, lotto.stream()
                                .map(String::valueOf)
                                .collect(Collectors.joining(",")));
  }
  ...
  private static Stream<Arguments> provideNotSortedLottoNumbers () {
    return Stream.of(
      Arguments.of(
        me.cleancode.lotto.step2.domain.LottoGenerator.generateLotto("5,6,4,1,2,10"),
        "1,2,4,5,6,10"
      ),
      Arguments.of(
        me.cleancode.lotto.step2.domain.LottoGenerator.generateLotto("6,5,4,3,2,1"),
        "1,2,3,4,5,6"
      )
    );
  }
  ...
  void 로또_번호_비교_테스트 (me.cleancode.lotto.step2.domain.Lotto lotto1, Lotto lotto2, int expected) {
  ...
  private static Stream<Arguments> provideTwoLottoAndSames () {
    return Stream.of(
      Arguments.of(
        me.cleancode.lotto.step2.domain.LottoGenerator.generateLotto("1,2,3,4,5,6"),
        me.cleancode.lotto.step2.domain.LottoGenerator.generateLotto("1,2,3,7,8,9"),
        3
      ),
      Arguments.of(
        me.cleancode.lotto.step2.domain.LottoGenerator.generateLotto("1,2,3,4,5,6"),
