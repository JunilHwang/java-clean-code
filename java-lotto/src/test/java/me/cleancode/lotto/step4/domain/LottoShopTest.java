...
import me.cleancode.lotto.step4.execption.LottoLesserPriceException;
import me.cleancode.lotto.step4.execption.LottoMinimumPriceException;
...
public class LottoShopTest {
  ...
  void 구매_금액_검증_테스트 (long price) {
    assertThatExceptionOfType(LottoMinimumPriceException.class)
      .isThrownBy(() -> LottoShop.buyLotto(price, new ArrayList<>()));
  }
  ...
  void 구매_금액_초과_테스트 (long price, List<Lotto> directLottos) {
    assertThatExceptionOfType(LottoLesserPriceException.class)
      .isThrownBy(() -> LottoShop.buyLotto(price, directLottos));
  }
  ...
  void 구매_갯수_테스트 (long price, long expected) {
    Assertions.assertEquals(expected, LottoShop.buyLotto(price, new ArrayList<>()).stream().count());
  }
